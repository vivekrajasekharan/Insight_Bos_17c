...
def is_substring_helper (data):
	#YOUR CODE GOES HERE
    #try all substrings between size 1 and len(data)/2
    for i in range(len(data)//2):
        substr = data[0:(i+1)]#We only care about substrings that start the word
        if data.count(substr)*len(substr) == len(data): 
        #if the count of the substr times its len is equal to the len of the data, then we're done 
            return(True)
    return(False)
